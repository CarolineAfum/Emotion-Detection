# ============================
# train_model.py
# Run this ONCE (locally or in a notebook) to train the model
# and save the artifacts that app.py will load for deployment.
# ============================
import os
import re
...
def load_split(name: str) -> pd.DataFrame:
    """Load a split from local parquet, falling back to the HF Hub if the
    local files aren't present (this is the most common source of the
    FileNotFoundError people hit here)."""
...
# Also was never called before - run it so you actually see the metrics.
page3()

# ============================
# 6b. Save evaluation results + an embeddings demo for the Streamlit app
# ============================
# Before, the reports/confusion matrices only got printed to the terminal
# during training and nothing was ever saved for app.py to load - that's
# why the "Model Evaluation" page was just placeholder text.
